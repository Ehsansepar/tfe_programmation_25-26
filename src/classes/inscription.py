...
class Inscription:
    ...
    def run_inscription(self, *args):
        ...
        while True:
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if self.rect_enter.collidepoint(event.pos):
                        if self.user_username != '' and self.user_pass != '':
                            if save_manager.register_user(self.user_username, self.user_pass):
            ...
            self.afficher_text("INSCRIPTION", self.police_titre, (255, 109, 0), config.WIDTH // 2, 100)

            col_user = (255, 109, 0) if self.rect_active_username else (100, 100, 100)
            col_pass = (255, 109, 0) if self.rect_active_password else (100, 100, 100)
            ...
            if hover_enter:
                ...
                self.afficher_text("S'inscrire", self.button_police, (15, 20, 30), self.rect_enter.centerx, self.rect_enter.centery)
            else:
                self.draw_neon_rect(self.ecran, col_enter, self.rect_enter, width=3, neon_strength=3)
                self.afficher_text("S'inscrire", self.button_police, col_enter, self.rect_enter.centerx, self.rect_enter.centery)

            if self.show_error:
                self.afficher_text("Erreur : Pseudo pris ou vide", self.button_police, (255, 23, 68), config.WIDTH // 2, 580)
